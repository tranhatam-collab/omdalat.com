...
OMDALAT is:
- a city node inside the OMDALA global system
...
Naming standard for SEO and content:

- display brand: `OMDALAT`
- Vietnamese readable name: `Ôm Đà Lạt`
- English readable / SEO name: `Om Dalat`
...
```
Ôm Đà Lạt | OMDALAT - Da Lat city node for work, creation, and trusted local value
Om Dalat | OMDALAT - The OMDALA city node in Da Lat for work, creation, and proof-backed activity
```
...
and real-world activity into a clearer entry path.
```
...
Optional when the authenticated product is mounted under the same host:

add `Disallow: /app/` only to that same-host `omdalat.com` robots file.

## 15.3 App host rule

If the authenticated product is deployed on `https://app.omdala.com`, block indexing on that host directly through:

- host-level `robots.txt`
- `noindex` or `X-Robots-Tag: noindex`
- no public sitemap entries for authenticated app URLs

The `/app/` disallow rule only applies when the authenticated experience lives under the same host as `omdalat.com`.

## 15.4 robots meta usage
