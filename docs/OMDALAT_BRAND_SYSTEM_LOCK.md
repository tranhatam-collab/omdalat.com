...
## Version 2.0 — Global / City Node Lock

---

# 1. PURPOSE OF THIS FILE

This document locks the brand hierarchy between:

- `OMDALA` as the global system
- `OMDALAT` as the Da Lat city node

It exists to ensure:
- one naming logic
- one hierarchy between master system and city node
- one visual direction for the public city layer
- one interaction tone across public web, docs, and app references
...
No team member should invent a new naming direction, hierarchy, or visual behavior outside this lock unless approved at brand system level.
...
OMDALA is the global system.

It owns:
- the global operating logic
- docs layer
- app layer
- trust framework
- cross-city system meaning

## 2.2 OMDALAT

OMDALAT is the Da Lat city node of OMDALA.

It owns:
- the public city-layer web surface
- local positioning in Da Lat
- local programs, places, hosts, and community narrative
- the entry point that routes people into the global docs and app layers

OMDALAT is not:
- the global app brand
- the global docs brand
- a standalone master system
...
- Master brand: `OMDALA`
- City node display brand: `OMDALAT`
- Vietnamese standard: `Ôm Đà Lạt`
- English standard: `Om Dalat`

Use `OMDALA` when referring to:
- docs
- app runtime
- global system logic
- cross-node architecture

Use `OMDALAT` when referring to:
- the public web at `omdalat.com`
- Da Lat positioning
- local city-node experience
- programs, hosts, places, and community context in Da Lat
...
**Primary statement:**
> OMDALAT is the Da Lat city node of OMDALA, turning local people, places, and activity into a trusted entry layer.

**Short version:**
> Trusted places, people, and activity in Da Lat.

**Alternative approved positioning lines:**
- OMDALAT is the Da Lat entry node of the OMDALA system.
- OMDALAT is a local interface for trusted activity, places, and people.
- OMDALAT brings nature, technology, and human value into one city-layer entry point.
...
- the public face of a bigger trusted system
...
# 20. APP-LEVEL EXPRESSION (app.omdala.com)

Must feel like:
- OMDALA runtime layer
- global system interface carrying city-node action
