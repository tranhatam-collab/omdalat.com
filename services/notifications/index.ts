...
function localizeNotification(notification: NotificationRecord, locale: OmdalatLocale): NotificationRecord {
  ...
  const mappedTitle: Record<string, string> = {
    ...
    "Basic account created": "Tài khoản cơ bản đã được tạo",
    "Application submitted": "Hồ sơ đã được gửi",
    "Member review updated": "Trạng thái review thành viên đã được cập nhật",
  ...
  const mappedDetail = notification.detail
    ...
    .replace("can now continue into the application flow.", "hiện có thể đi tiếp vào luồng hồ sơ.")
    .replace("is now in the", "hiện đang ở lớp")
    .replace("lane.", ".")
    .replace("moved to", "đã được chuyển sang")
